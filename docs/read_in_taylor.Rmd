...
combined_data2 = combined_data %>% filter(sample_date >= "1997-01-08")
...
no3 = ggplot(combined_data2,
  ...
  geom_line(aes(linetype = stream_sample),
            size = 0.4) +
        labs(x = "Year",
             y = "NO3 concentration (μg/l)", 
             title = "NO3 & K Concentrations in Stream Water from the 
             Luquillo Mountains (1986-2012)", size = 4) +
...
k = ggplot(combined_data2,
  ...
  geom_line(aes(linetype = stream_sample),
        ...
        labs(x = "Year",
             ...
             caption = "Weekly concetrates of potassium and nitrate in from sites in the Lucillo Mountains. 
             NO3 (μg/l) = Nitrate concentration per microgram per liter; K (mg/l) = miligram per liter;
